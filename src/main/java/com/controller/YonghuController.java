    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(yonghu !=null){
            ...
            BeanUtils.copyProperties( yonghu , view );//把实体数据重构到view中
                ...
                //级联表
                ZhibuEntity zhibu = zhibuService.selectById(yonghu.getZhibuId());
                if(zhibu != null){
    ...
    */
    ...
    public R save(@RequestBody YonghuEntity yonghu, HttpServletRequest request){
        ...
        if(false)
            ...
            yonghu.setZhibuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        if(yonghuEntity==null){
            ...
            return R.error(511,"账户或者联系方式或者党员身份证号已经被使用");
        }
    }
...
//                            yonghuEntity.setYonghuName(data.get(0));                    //党员姓名 要改的
//                            yonghuEntity.setYonghuPhoto("");//照片
//                            yonghuEntity.setMinzu(data.get(0));                    //民族 要改的
//                            yonghuEntity.setSexTypes(Integer.valueOf(data.get(0)));   //性别 要改的
//                            yonghuEntity.setJiguan(data.get(0));                    //籍贯 要改的
//                            yonghuEntity.setChushengriqi(new Date(data.get(0)));          //出生日期 要改的
//                            yonghuEntity.setWenhuachengdu(data.get(0));                    //文化程度 要改的
//                            yonghuEntity.setGongzuodanwei(data.get(0));                    //工作单位 要改的
//                            yonghuEntity.setGongzuozhiwu(data.get(0));                    //工作职务 要改的
//                            yonghuEntity.setRudangshijian(new Date(data.get(0)));          //入党时间 要改的
//                            yonghuEntity.setZhuanzhengshijian(new Date(data.get(0)));          //转正时间 要改的
//                            yonghuEntity.setZhibuId(Integer.valueOf(data.get(0)));   //所在支部 要改的
//                            yonghuEntity.setQianfariqi(data.get(0));                    //签发日期 要改的
//                            yonghuEntity.setYonghuPhone(data.get(0));                    //联系方式 要改的
//                            yonghuEntity.setYonghuIdNumber(data.get(0));                    //党员身份证号 要改的
...
//                            yonghuEntity.setCreateTime(date);//时间
                        ...
                        if(yonghuEntities_yonghuIdNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [党员身份证号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
    ...
    */
    ...
    public R login(String username, String password, String captcha, HttpServletRequest request) {
        ...
        // role_types.get(zhibu.getRoleTypes());
        String token = tokenService.generateToken(yonghu.getId(),username, "yonghu", "党员");
        R r = R.ok();
        r.put("token", token);
        r.put("role","党员");
...
//    	ValidatorUtils.validateEntity(user);
        ...
        YonghuEntity yonghuEntity = yonghuService.selectOne(queryWrapper);
        if(yonghuEntity != null)
            return R.error("账户或者联系方式或者党员身份证号已经被使用");
        yonghu.setYonghuDelete(1);
        yonghu.setCreateTime(new Date());
        yonghuService.insert(yonghu);
        return R.ok();
    ...
    public R getCurrYonghu(HttpServletRequest request){
        ...
        if(yonghu !=null){
            ...
            //级联表
                            ZhibuEntity zhibu = zhibuService.selectById(yonghu.getZhibuId());
            if(zhibu != null){
                BeanUtils.copyProperties( zhibu , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                view.setZhibuId(zhibu.getId());
            }
            //修改对应字典表字段
            dictionaryService.dictionaryConvert(view, request);
